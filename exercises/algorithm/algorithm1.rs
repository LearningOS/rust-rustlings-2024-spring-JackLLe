...
T:Clone+std::cmp::PartialOrd ,
 {
	...
	pub fn merge( list_a: LinkedList<T>,  list_b: LinkedList<T>) -> Self {
        ...
        while let (Some(ptr_a), Some(ptr_b)) = (node_a, node_b) {
            let val_a = unsafe { ptr_a.as_ref().val.clone() };
            let val_b = unsafe { ptr_b.as_ref().val.clone() };
        ...
        while let Some(ptr_a) = node_a {
            merged_list.add(unsafe { ptr_a.as_ref().val.clone() });
            node_a = unsafe { ptr_a.as_ref().next };
        }

        while let Some(ptr_b) = node_b {
            merged_list.add(unsafe { ptr_b.as_ref().val.clone() });
            node_b = unsafe { ptr_b.as_ref().next };
        }
